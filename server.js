...
async function sendEmails(Reminder) {
  ...
  const firstJanSinceEpoch = Math.ceil(
    new Date(`1 1 ${new Date().getFullYear()}`).getTime() / 86400000
  );
  const currDays = currDate - firstJanSinceEpoch + 1;

  //fix dates in Jan (not to skipp reminders from the beginning of the next year)
  const fixedReminders = reminders.map((r) =>
    r.eventInDays - r.interval < 0
      ? { ...r, eventInDays: r.eventInDays + currDays }
      : r
  );

  //get only those to remind now
  const toRemind = fixedReminders.filter(
    (r) => r.eventInDays - r.interval == currDays
  );
